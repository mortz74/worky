...
const NavIcon = ({ path }) => {
  const icons = {
    ...
    files:     <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="17" height="17"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>,
    reminders: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="17" height="17"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></svg>,
...
export default function Sidebar({ mobileOpen = false, onMobileClose = () => {} }) {
  ...
  const upcomingReminders = data.reminders.filter(r => new Date(r.remindAt) >= new Date()).length
  ...
  const navItems = [
    ...
    { path: '/reminders',  label: 'Reminders', badge: upcomingReminders },
    { path: '/assignees',  label: 'Assignees', section: 'PEOPLE' },
    { path: '/settings',  label: 'Settings',  section: 'SYSTEM' },
  ]

  // On mobile drawer, always show expanded regardless of collapsed state
  const isCollapsed = collapsed && !mobileOpen

  return (
    <div className={`sidebar${isCollapsed ? ' sidebar-collapsed' : ''}${mobileOpen ? ' sidebar-mobile-open' : ''}`}>
      <div className="sidebar-logo" onClick={toggleCollapsed} title={isCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}>
        ...
        {!isCollapsed && <span className="logo-text">Worky</span>}
      </div>

      <nav className="sidebar-nav">
        {navItems.map((item, i) => (
          <div key={item.path}>
            {!isCollapsed && item.section && <div className="nav-section">{item.section}</div>}
            <NavLink
              to={item.path}
              title={isCollapsed ? item.label : ''}
              className={({ isActive }) => `nav-item${isActive ? ' active' : ''}${isCollapsed ? ' nav-item-collapsed' : ''}`}
              onClick={onMobileClose}
            >
              <NavIcon path={item.path.slice(1)} />
              {!isCollapsed && <span>{item.label}</span>}
              {!isCollapsed && item.badge > 0 && <span className="nav-badge">{item.badge}</span>}
              {isCollapsed && item.badge > 0 && <span className="nav-badge-dot"></span>}
            </NavLink>
          </div>
        ))}
      </nav>
